...
TEST(AppKeymapTest, add_app) {
  ...
  int modcode = Shift_L;
  EXPECT_EQ(add_app(TEST_APP, modcode, 0, NULL), 0);
...
TEST(AppKeymapTest, add_app_multi_for_mod) {
  ...
  int modcode = Shift_L;
  EXPECT_EQ(add_app(TEST_APP, modcode, 0, NULL), 0);
  EXPECT_EQ(add_app(SECOND_APP, modcode, 0, NULL), 0);
...
TEST(AppKeymapTest, add_app_invalid_name) {
  ...
  EXPECT_EQ(add_app("", Alt, 0, NULL), -1);
}

TEST(AppKeymapTest, add_app_already_exists) {
  clear_apps();

  EXPECT_EQ(add_app(TEST_APP, Alt, 0, NULL), 0);
  EXPECT_EQ(add_app(TEST_APP, Shift, 0, NULL), -1);
}
...
TEST(AppKeymapTest, find_app) {
  ...
  EXPECT_EQ(add_app(SECOND_APP, Control, 0, NULL), 0);
...
TEST(AppKeymapTest, find_app_multi_for_mod) {
  clear_apps();
  EXPECT_EQ(add_app(TEST_APP, Alt, 0, NULL), 0);
  EXPECT_EQ(add_app(SECOND_APP, Alt, 0, NULL), 0);
...
TEST(AppKeymapTest, delete_app_multi_for_mod_first) {
  ...
  EXPECT_EQ(add_app(TEST_APP, Alt, 0, NULL), 0);
  EXPECT_EQ(add_app(SECOND_APP, Alt, 0, NULL), 0);
  EXPECT_EQ(add_app(THIRD_APP, Alt, 0, NULL), 0);
